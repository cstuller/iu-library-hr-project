<?php
/**
 * Contains the function calls to create the page to show applicants for a posting
...
if(!empty($_GET['ID'])){
  ...
  if(!empty($results) && $results['Approved'] == TRUE && $results['Owner'] == $_ENV["REMOTE_USER"] && $results['Enabled'] == TRUE){
    $appTable = buildSupervisorApplicantTable($id);
    $page.= buildSupervisorApplicantList($results['Title'], $appTable, $id);
  }
  else{
